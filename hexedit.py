...
INVENTORY_ENTRY_COUNT = 2048
INVENTORY_SEARCH_START = 30000
INVENTORY_SEARCH_END = 195000
INVENTORY_SAMPLE_ENTRIES = 192
...
def _inventory_quantity_length(raw_item):
    return 2 if raw_item <= 0xFFFF else 4


def _encode_quantity(quantity, raw_item):
    size = _inventory_quantity_length(raw_item)
    return int(quantity).to_bytes(size, "little")


def normalize_item_descriptor(item_value):
    if isinstance(item_value, dict):
        if isinstance(item_value.get("raw_item"), int):
            raw_item = item_value["raw_item"]
        elif isinstance(item_value.get("goods_id"), int):
            raw_item = GOODS_FLAG | item_value["goods_id"]
        else:
            ids = item_value.get("ids")
            raw_item = GOODS_FLAG | (ids[0] + (ids[1] << 8))
    elif isinstance(item_value, (list, tuple)) and len(item_value) == 2:
        raw_item = GOODS_FLAG | (int(item_value[0]) + (int(item_value[1]) << 8))
    elif isinstance(item_value, int):
        raw_item = item_value
    else:
        raise ValueError(f"Unsupported item descriptor: {item_value!r}")

    return {
        "raw_item": int(raw_item),
        "item_id": [raw_item & 0xFF, (raw_item >> 8) & 0xFF],
        "goods_id": raw_item & 0x0FFFFFFF,
    }


def _known_inventory_items():
    known = {}

    for item_name, ids in itemdict.items():
        raw_item = GOODS_FLAG | (ids[0] + (ids[1] << 8))
        known.setdefault(raw_item, item_name)

    database = itemdata.load_item_database()
    for category in database.values():
        if not isinstance(category, dict):
            continue
        for item_name, item_value in category.items():
            descriptor = normalize_item_descriptor(item_value)
            known.setdefault(descriptor["raw_item"], item_name)

    return known


KNOWN_INVENTORY_ITEMS = _known_inventory_items()


def _read_inventory_entry(slot_data, index):
...
def _is_valid_inventory_entry(raw_item, quantity, unique_id):
    if quantity <= 0:
        return False
    if raw_item == 0:
        return False
...
def _iter_inventory_offsets(slot_data):
    end = min(len(slot_data) - INVENTORY_ENTRY_SIZE, INVENTORY_SEARCH_END)
    for index in range(INVENTORY_SEARCH_START, end + 1, 4):
        yield index


def write_save_bytes(file, data, backup_label="save-write"):
    savefile_io.prepare_for_write(file, label=backup_label)
    with open(file, "wb") as handle:
        handle.write(data)


def find_inventory_start(file, slot):
    slot_data = get_slot_ls(file)[slot - 1]
    best = None
    max_start = min(len(slot_data) - (INVENTORY_ENTRY_SIZE * INVENTORY_SAMPLE_ENTRIES), INVENTORY_SEARCH_END)

    for start in range(INVENTORY_SEARCH_START, max_start, 4):
        known_count = 0
        valid_count = 0

        for offset in range(start, start + (INVENTORY_ENTRY_SIZE * INVENTORY_SAMPLE_ENTRIES), INVENTORY_ENTRY_SIZE):
            raw_item, quantity, uid = _read_inventory_entry(slot_data, offset)
            if _is_valid_inventory_entry(raw_item, quantity, uid):
                valid_count += 1
            if raw_item in KNOWN_INVENTORY_ITEMS:
                known_count += 1

        score = (known_count * 8) + valid_count
        candidate = {"start": start, "score": score, "known_count": known_count, "valid_count": valid_count}
        if best is None or candidate["score"] > best["score"]:
            best = candidate

    return best["start"] if best is not None else INVENTORY_SEARCH_START
    ...
    def replacer(file, old_name, name):
        ...
        with open(file, "rb") as fh:
            ...
            for i in id_loc:
                ...
                write_save_bytes(file, data, backup_label="change-name")
...
def replace_id(file, newid):
    ...
    with open(file, "rb") as f:
        ...
        for i in id_loc:
            ...
            write_save_bytes(file, data, backup_label="replace-steam-id")
...
def copy_save(src_file, dest_file, src_char, dest_char):
    ...
    write_save_bytes(dest_file, dat1, backup_label="copy-save")
...
def set_stats(file, char_num, stat_ls):
    ...
    for loc in locs:
        ...
        if index == 8:
            ...
            write_save_bytes(file, data, backup_label="set-stats")
            break
...
def set_level(file, char, lvl):
    ...
    write_save_bytes(file, data, backup_label="set-level")
    recalc_checksum(file)
...
def additem(file, slot, itemids, quantity):
    ...
    descriptor = normalize_item_descriptor(itemids)
    quantity_bytes = _encode_quantity(quantity, descriptor["raw_item"])

    for ind in _iter_inventory_offsets(cs):
        raw_item, cur_quantity, uid = _read_inventory_entry(cs, ind)
        if not _is_valid_inventory_entry(raw_item, cur_quantity, uid):
            continue
        if raw_item != descriptor["raw_item"]:
            continue

        quantity_start = ind + 4
        quantity_end = quantity_start + len(quantity_bytes)
        ch = s_start + cs[:quantity_start] + quantity_bytes + cs[quantity_end:] + s_end
        write_save_bytes(file, ch, backup_label="add-item")
...
#        dat = f.read()
    ...
    write_save_bytes(file, ch, backup_label="set-starting-class")
...
def find_inventory(file,slot,ids):
    descriptor = normalize_item_descriptor(ids)
    c1 = get_slot_ls(file)[slot - 1]

    for ind in _iter_inventory_offsets(c1):
        raw_item, quantity, uid = _read_inventory_entry(c1, ind)
        if not _is_valid_inventory_entry(raw_item, quantity, uid):
            continue
        if raw_item == descriptor["raw_item"]:
            return ind
...
def get_inventory(file, slot):
    ...
    for ind in _iter_inventory_offsets(c1):
        ...
        name = KNOWN_INVENTORY_ITEMS.get(raw_item, "?")
        ls.append(
            {
                "name": name,
                "item_id": [raw_item & 0xFF, (raw_item >> 8) & 0xFF],
    ...
    return [entry for entry in sorted_ls if entry["name"] != "?"]
...
def overwrite_item(file,slot, item_dict_entry, newids):
    ...
    descriptor = normalize_item_descriptor(newids)
    raw_item_bytes = descriptor["raw_item"].to_bytes(4, "little")
    cs = get_slot_ls(file)[slot - 1]
    slices = get_slot_slices(file)
    s_start = slices[slot - 1][0]
    s_end = slices[slot - 1][1]

    ch = s_start + cs[:pos] + raw_item_bytes + cs[pos + 4 :] + s_end
    write_save_bytes(file, ch, backup_label="overwrite-item")
...
def fix_stats(file, char_slot, stat_list):
    ...
    write_save_bytes(file, data, backup_label="fix-stats")
